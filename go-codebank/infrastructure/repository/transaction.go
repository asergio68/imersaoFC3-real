...
func (t *TransactionRepositoryDb) SaveTransaction(transaction domain.Transaction, creditCard domain.CreditCard) error {
	...
	if transaction.Status == "approved" {
		err = t.UpdateCreditCardBalance(creditCard)
		if err != nil {
			return err
		}
	}
...
func (t *TransactionRepositoryDb) UpdateCreditCardBalance(creditCard domain.CreditCard) error {
...
func (t *TransactionRepositoryDb) CreateCreditCard(creditCard domain.CreditCard) error {
	stmt, err := t.db.Prepare(
		`insert into credit_cards (id, name, number, expiration_month, expiration_year, cvv, balance,
				limit, created_at)
